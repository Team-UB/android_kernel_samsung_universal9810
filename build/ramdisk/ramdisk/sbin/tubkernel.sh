...
APKINSTALLDIR="/data/.tub/apk_install"
...
# Install APK Files (@Morogoku)
if [ ! -d $APKINSTALLDIR ]; then
	mkdir -p $APKINSTALLDIR;
	chown -R root.root $APKINSTALLDIR;
	chmod 750 $APKINSTALLDIR;
fi
(
if [ "$(ls -A /$APKINSTALLDIR)" ]; then
	cd $APKINSTALLDIR
	chmod 777 *
	for APK in *.apk; do
		echo "Installing $APK"
		su -c "pm install -r $APKINSTALLDIR/$APK"
		rm $APK
	done;
fi
) 2>&1	 | tee -a /data/.tub/log.log
